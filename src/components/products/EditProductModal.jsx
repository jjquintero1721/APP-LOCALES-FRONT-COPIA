/**
 * Modal para editar un Producto/Receta existente
 * Pre-puebla los datos del producto y sus ingredientes
 */
import React, { useState, useEffect, useMemo } from 'react';
import { useForm } from 'react-hook-form';
import { useProducts, useProduct } from '../../hooks/products/useProducts';
import IngredientsSelector from './IngredientsSelector';
import { useInventory } from '../../hooks/inventory/useInventory';
import './ProductModals.css';

const EditProductModal = ({ isOpen, onClose, product }) => {
  const { updateProduct, updateIngredients, isUpdating, isUpdatingIngredients } = useProducts();
  const { inventoryItems } = useInventory(0, 100, true);
  const [selectedIngredients, setSelectedIngredients] = useState([]);
  const [ingredientsError, setIngredientsError] = useState('');

  // Cargar el producto completo con ingredientes
  const { product: fullProduct, isLoading: isLoadingProduct } = useProduct(product?.id, isOpen && !!product?.id);
  ...
  // Pre-poblar formulario cuando el producto completo se carga
  useEffect(() => {
    if (productData && isOpen && !isLoadingProduct) {
      reset({
        name: productData.name || '',
        description: productData.description || '',
        category: productData.category || '',
        sale_price: productData.sale_price || '',
        profit_margin_percentage: productData.profit_margin_percentage || '',
      ...
      if (productData.ingredients && productData.ingredients.length > 0) {
        const formattedIngredients = productData.ingredients.map(ing => ({
          inventory_item_id: ing.inventory_item_id,
          quantity: ing.quantity,
          id: ing.id // Mantener el ID del ingrediente para edición
        }));
  ...
  }, [productData, isOpen, isLoadingProduct, reset]);
  ...
  const onSubmit = async (data) => {
    ...
    // Preparar datos del producto (SIN ingredientes)
    const productData = {
      name: data.name,
    ...
    updateProduct(
      { productId: product.id, data: productData },
      {
        onSuccess: () => {
          // Luego actualizar los ingredientes
          updateIngredients(
            { productId: product.id, ingredients: ingredientsData },
            {
              onSuccess: () => {
                onClose();
              }
      ...
      <div className="modal-content modal-lg" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="modal-form">
          ...
          <div className="modal-footer">
            <button type="button" onClick={onClose} className="btn-secondary" disabled={isUpdating || isUpdatingIngredients}>
              Cancelar
            </button>
            <button type="submit" className="btn-primary" disabled={isUpdating || isUpdatingIngredients}>
              {(isUpdating || isUpdatingIngredients) ? 'Actualizando...' : 'Guardar Cambios'}
            </button>
          </div>
        </form>
      </div>
